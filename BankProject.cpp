...
stUser currentUser;
stUser defaultAdmin = { "Admin","1111",-1 };
enum class  enOperation { nothing, clientsList, addClients, deleteClient, updateClient, findClient, transaction, manageUsers, logout };
enum class enTransaction { deposit = 1, withdraw, totalBalances, mainMenu };
enum class enPermission { clientsList = 1, addClients = 2, deleteClient = 4, updateClient = 8, findClient = 16, transaction = 32, manageUsers = 64 };
...
void ShowWithdrawScreen() {
    ...
    do {
        ...
        if (ClientIsExistsInFileByAccNum(clientsFileName, separator, accountNumber, c1)) {
            ...
            double amount;
            do {
                amount = GetAmount("Please enter withdraw amount ?");
...
void ShowDeniedAccessScreen() {
    ...
    cout << "-------------------------------------\n";
    cout << "Access Denied,\n";
    cout << "You donot have permission to do this,\n";
    cout << "Please contact your admin .\n";
    cout << "-------------------------------------\n";
}
bool HavePermission(enOperation& oper) {
     switch (oper)
  {
  case enOperation::logout:
      return true;
  case  enOperation::clientsList:
      if ((currentUser.permission & (short) enPermission::clientsList) == (short)enPermission::clientsList) {
          return true;
      }
      break;
  case enOperation::addClients:
      if ((currentUser.permission & (short)enPermission::addClients) == (short)enPermission::addClients) {
          return true;
      }
      break;
  case enOperation::deleteClient:
      if ((currentUser.permission& (short) enPermission::deleteClient) == (short) enPermission::deleteClient) {
          return true;
      }
      break;
  case enOperation::updateClient:
      if ((currentUser.permission & (short) enPermission::updateClient) == (short) enPermission::updateClient) {
          return true;
      }
      break;
  case enOperation::findClient:
      if ((currentUser.permission & (short) enPermission::findClient) == (short) enPermission::findClient) {
          return true;
      }
      break;
  case enOperation::transaction:
      if ((currentUser.permission & (short)enPermission::transaction) == (short)enPermission::transaction) {
          return true;
      }
      break;
  case enOperation::manageUsers:
      if ((currentUser.permission & (short)enPermission::manageUsers) == (short)enPermission::manageUsers) {
          return true;
      }
      break;
  default:
...
void ExecuteOperation(enOperation& oper) {
    if (HavePermission(oper)) {
        if (oper != enOperation::transaction && oper != enOperation::manageUsers) {
            system("cls");
        }
        switch (oper)
        {
     ...
     if (oper != enOperation::logout && oper != enOperation::transaction && oper != enOperation::manageUsers) {
...
void OpenBankSystem() {
    ...
    do {
        system("cls");
        PrintMainMenu();
        operationNumber = (enOperation)PositiveShortNumberInRange("Choose what do you want to do? [1 to 8]?\n", 1, 8);
        ExecuteOperation(operationNumber);
    } while (operationNumber != enOperation::logout);

}
stUser ConvertLineToUserRecord(const string& s) {
    vector <string> split = SplitString(s, separator);
    stUser user;
...
string ConvertUserRecordToLine(const stUser& user) {
    return(user.userName + separator + user.password + separator + to_string(user.permission));
}
...
void SaveUnMarkedVUsersInFile(const vector <stUser>& users) {
    fstream myfile;

    myfile.open(UsersFileName, ios::out);
    if (myfile.is_open()) {
        for (const stUser& user : users) {
            if (user.markAsDelete == false) {
...
bool IsAdminInUsersList(const vector <stUser>& users) {
    if (users.size() > 0) {
        for (const stUser& user : users) {
...
void EnsureAdminExistAndSetup() {
    vector<stUser> users = LoadUsersDataFromFile();
    if (!IsAdminInUsersList(users)) {
        users.insert(users.begin(), defaultAdmin);
        SaveUnMarkedVUsersInFile(users);
    }

}
bool SearchUserInUserFileByUsername(const string& username, stUser& user) {
    fstream myfile;
    string line;
    stUser u;
    ...
    if (myfile.is_open()) {
        while (getline(myfile, line)) {
            u = ConvertLineToUserRecord(line);
...
void PrintOneUserInList(const stUser& user) {
    cout << "| " << left << setw(20) << user.userName;
    cout << "| " << left << setw(20) << user.password;
    cout << "| " << left << setw(40) << user.permission << endl;
}
void PrintUserList() {
    vector <stUser> users = LoadUsersDataFromFile();
    ...
    cout << "| " << left << setw(40) << "Permission" << endl;
    cout << "_______________________________________________________________________________________\n";
    for (const stUser& user : users)
    {
        PrintOneUserInList(user);
    }
    cout << "_______________________________________________________________________________________\n\n";

}
bool UserExistInUsersVectorbyUsername(const vector <stUser> &users,string username) {
for(const stUser & user : users){
...
short AskForPermission(){
    short calcpermission=0 ;
    if(YesOrNo("Do you want to give full access ? ")){
      calcpermission = -1 ;
    }else{
      cout<<"Do you Want to give access to :\n";
      calcpermission += (YesOrNo("Show Client List ? "))? (short)enPermission::clientsList : 0;
      calcpermission += (YesOrNo("Add New Clients ? "))? (short)enPermission::addClients : 0;
      calcpermission += (YesOrNo("Delete Client ? "))? (short)enPermission::deleteClient : 0;
      calcpermission += (YesOrNo("Update Client ? "))? (short)enPermission::updateClient : 0;
      calcpermission += (YesOrNo("Find Client ? "))? (short)enPermission::findClient : 0;
      calcpermission += (YesOrNo("Transactions ? "))? (short)enPermission::transaction : 0;
      calcpermission += (YesOrNo("Manage Users ? "))? (short)enPermission::manageUsers : 0;
    }
     return calcpermission ;

}
stUser ReadNewUser (const vector <stUser> &users){
 stUser newUser ;
 
 while (true){  
    newUser.userName=GetStringInSize("Enter User Name : ",3,10,"Please Try Again, User Name Must be in range from 3 to 10 character : ");
    if(UserExistInUsersVectorbyUsername(users,newUser.userName)){
        cout<<"User with ["<<newUser.userName<<"] already exist, Try again with another.\n";
    }else{
        break;
    }
 } ;
 newUser.password=GetStringInSize("Enter Password : ",4,20,"Please Try Again, Password Must be in range from 4 to 20 character : ");
 newUser.permission=AskForPermission() ;
 return newUser ;
...
if(myfile.is_open()){
    do{
        newuser=ReadNewUser(users);
        myfile<<ConvertUserRecordToLine(newuser)<<endl;
        users.push_back(newuser);    
       cout << "\nThe User has been added successfully. :-)\n";
    }while (YesOrNo("Do you want to add more users ? Yes/No ? "));
   myfile.close();
}
...
void DeleteUserFromFileByUserName(const string &username){
    vector <stUser> users =LoadUsersDataFromFile();
    for(stUser & user : users ){
        if(user.userName==username){
            user.markAsDelete=true;
            break;
        }
    }
    SaveUnMarkedVUsersInFile(users);
...
cout<<"            Delete User\n";
cout<<"=====================================\n\n";
string username =GetStringInSize("Enter User Name : ",1,40,"Writing this number of characters is not allowed. Please try entering the username again : ");
stUser user ;
if(username=="Admin"){
    cout<<"Sorry, You can not delete Admin .\n";

}else {
     if(SearchUserInUserFileByUsername(username,user)){
        cout<<"The following are the user details : \n";
        PrintUserCard(user);
        if ( YesOrNo("Are you sure you want delete this user ? Yes/No? ")){
            DeleteUserFromFileByUserName(username);
...
stUser InputUpdateUserData(string username){
    stUser newdata ;
    newdata.userName=username ;
    if(username=="Admin"){
      newdata.permission=-1;
      newdata.password=GetStringInSize("Enter Password : ",4,20,"Please Try Again, Password Must be in range from 4 to 20 character : ");
    }else{
         newdata.password=GetStringInSize("Enter Password : ",4,20,"Please Try Again, Password Must be in range from 4 to 20 character : ");
         newdata.permission=AskForPermission() ;
    }
    return newdata ;
}
void UpdateUserFromFileByUserName(string username){
vector <stUser> users = LoadUsersDataFromFile();
stUser newdata =InputUpdateUserData(username);
for(stUser &user :users){
    if(user.userName==username){
        user=newdata;
        break;
...
cout<<"            Update User\n";
cout<<"=====================================\n\n";
string username =GetStringInSize("Enter User Name : ",1,200);
stUser user ;
if(username=="Admin"){
    cout<<"This user is the admin; you can only change the password.\n\n";

} 
if(SearchUserInUserFileByUsername(username,user)){
    cout<<"The following are the user details : \n";
    PrintUserCard(user);
        if ( YesOrNo("Are you sure you want Update this user ? Yes/No? ")){
            UpdateUserFromFileByUserName(username);
...
void ExecuteManageUsersOperation(enManageUsers operation) {
    system("cls");
    switch (operation)
    {
...
void ShowManageUsersScreen() {
    ...
    do {
        ...
        operation = (enManageUsers)PositiveShortNumberInRange("Choose what do you want to do? [1 to 6]?\n", 1, 6);
        if (operation != enManageUsers::mainMenu)
            ExecuteManageUsersOperation(operation);
    } while (operation != enManageUsers::mainMenu);
}
bool LoadCurrentUserInfo() {
    stUser user;
    string username;
    string Password;
        username = GetStringInSize("Enter Username : ", 1, 40, "Writing this number of characters is not allowed. Please try entering the username again : ");
        Password = GetStringInSize("Enter Password : ", 1, 40, "Writing this number of characters is not allowed. Please try entering the password again : ");
        if (SearchUserInUserFileByUsername(username, user) && (user.password == Password)) {
            currentUser=user ;
            return true ;
        }
        else {
    ...
    while (true) {
      ...
      LoginFailed =false ;
       do{ system("cls");
         ...
         LoginFailed =!LoadCurrentUserInfo();
       }while (LoginFailed);
        OpenBankSystem( );
    }
}
